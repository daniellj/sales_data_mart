{{ config(
    ...
    tags=['countries'],
    description='Extracts and transforms country data by casting types, adding a continent classification based on ISO alpha-2 codes, and incrementally loading only new records based on the create_date_ts timestamp. This model ensures unique records by alpha_2 and alpha_3 codes.'
) }}
...
with source_data as (
    select
        ...
        CASE
            WHEN upper(alpha_2) IN ('DZ','DZ','BJ','BW','BF','BI','CM','CV','CF','TD','KM','CD','CG','CI','DJ','EG','GQ','ER','ET','GA','GM','GH','GN','GW','KE','LS','LR','LY','MG','MW','ML','MR','MU','MA','MZ','NA','NE','NG', 'RE', 'RW','ST','SN','SC','SL','SO','ZA','SS','SD','SZ','TZ','TG','TN','UG','EH','ZM','ZW', 'YT', 'SH') THEN 'Africa'
